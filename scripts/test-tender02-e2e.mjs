...
async function runTender02E2ETests() {
  console.log('================================================================');
  console.log('🚀 BẮT ĐẦU TEST E2E: ĐỢT MỜI CHÀO GIÁ #02 (SIMPLIFIED: 4 RÈM, LƯỚI, GIÀN PHƠI)');
  console.log('================================================================\n');
  ...
  try {
    ...
    const { data: quoteCols, error: quoteErr } = await supabase
      ...
      .select('item_type, unit, pricing_mode, list_price, discount_percent, effective_price, plan_name, fabric_main, fabric_sheer, wire_spec, wire_diameter_mm, wire_spacing_cm, frame_spec, load_capacity_kg, drying_bars_count')
      .limit(1);
    assert(!quoteErr, 'Bảng supplier_quotes có đủ các cột quy cách, chiết khấu');
    ...
    // -------------------------------------------------------------
    console.log('\n📋 [TEST 3] Kiểm tra Đợt Mời Chào Giá #02 chính thức...');
    let tender02Id = null;
    ...
    // -------------------------------------------------------------
    // TEST 4: Seed 6 Standard Items for Round #02 (No REM-CUON)
    // -------------------------------------------------------------
    console.log('\n📋 [TEST 4] Kiểm tra 6 hạng mục chuẩn cho Đợt #02 (Đã loại bỏ Rèm cuốn)...');
    const standardItems = [
      {
    ...
    // Ensure REM-CUON is removed
    await supabase.from('supplier_tender_items').delete().eq('tender_id', tender02Id).eq('model_code', 'REM-CUON');
    ...
    assert(itemsInT02 && itemsInT02.length === 6, 'Đúng 6 hạng mục chuẩn cho Đợt #02 (4 rèm, 1 lưới, 1 giàn phơi)');
    assert(!itemsInT02?.some(i => i.model_code === 'REM-CUON'), 'Đã loại bỏ hoàn toàn Rèm cuốn chống nắng khỏi Đợt #02');
    ...
    // -------------------------------------------------------------
    console.log('\n📋 [TEST 5] Gửi báo giá kiểm thử (Test Submissions & Single Price per Plan)...');
    ...
    // -------------------------------------------------------------
    console.log('\n📋 [TEST 6] Kiểm tra logic so sánh giá & highlight giá thấp nhất...');
    ...
    // -------------------------------------------------------------
    console.log('\n📋 [TEST 7] Kiểm tra hệ thống khảo sát cư dân...');
    ...
    assert(cleanedCount >= 3, 'Dọn dẹp sạch sẽ dữ liệu kiểm thử thành công');
